...
unsafe
{
    ...
    XAudio2.CreateWithVersionInfo(audio.GetAddressOf(), 0, XAudio2.XAudio2_USE_DEFAULT_PROCESSOR, 0).ThrowIf();
    ...
    X3DAudio.Initialize(channelMask, X3DAudio.X3DAudio_SPEED_OF_SOUND, &handle).ThrowIf();
    ...
    X3DAudio.Calculate(handle, &listener, &emitter, X3DAudio.X3DAudio_CALCULATE_MATRIX | X3DAudio.X3DAudio_CALCULATE_DOPPLER | X3DAudio.X3DAudio_CALCULATE_LPF_DIRECT | X3DAudio.X3DAudio_CALCULATE_REVERB, &settings);
    ...
    while (running)
    {
        ...
        X3DAudio.Calculate(handle, &listener, &emitter,
        ...
        XAudio2FilterParameters FilterParameters = new(FilterType.LowPassFilter, 2.0f * MathF.Sin(float.Pi / 6.0f * settings.LPFDirectCoefficient), 1.0f);
